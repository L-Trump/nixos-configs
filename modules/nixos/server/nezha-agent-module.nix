{
  ...
  cfg = config.services.nezha-agent-dev;
...
in {
          ...
          options = {
            ...
            uuid = lib.mkOption {
              ...
              description = ''
                ...
                Set {option}`services.nezha-agent-dev.genUuid` to true to generate uuid
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      {
        ...
        message = "Please set `service.nezha-agent-dev.settings.uuid` while `genUuid` is false.";
      }
      {
        assertion = cfg.settings.uuid != null -> !cfg.genUuid;
        message = "When `service.nezha-agent-dev.genUuid = true`, `settings.uuid` cannot be set.";
      }
    ];

    services.nezha-agent-dev.settings = {
      debug = cfg.debug;
    ...
    systemd.services.nezha-agent-dev = {
      serviceConfig = {
        Restart = "on-failure";
