...
currentVersion=$(nix-instantiate --eval -E "with import ./. {}; rustdesk-server-pro.version" | tr -d '"')
...
for i in \
    ...
    update-source-version rustdesk-server-pro "$latestVersion" "$hash" --system="$1" --ignore-same-version
done
